...
import { ArrowLeft, ArrowRight, CheckCircle2, Send, Loader2, Mail, Lock } from 'lucide-react';
import { Button } from '../../components/Button';
import { DealConfig, Signatory, UploadedFile, MOCK_OCR_DATA } from '../../types/types';
...
export const NewDealWizard: React.FC<{ onCancel: () => void, onFinish: () => void }> = ({ onCancel, onFinish }) => {
  ...
  const [configData, setConfigData] = useState<DealConfig>({
      name: '', contractModel: 'venda_compra_padrao', useFgts: false, bankFinancing: false, 
      consortiumLetter: false, sellersCount: 1, sellersType: 'PF', buyersCount: 1, buyersType: 'PF', deedCount: 1,
      isSpouse: false
  });
  const [documents, setDocuments] = useState<UploadedFile[]>([]);
  const [mappings, setMappings] = useState<Record<string, string>>({});
  const [signatories, setSignatories] = useState<Signatory[]>([]);
  const [ocrData, setOcrData] = useState<any>(MOCK_OCR_DATA);
             ...
             <AnimatePresence mode="wait" initial={false} custom={direction}>
                ...
                >
                    ...
                    {step === 3 && (
                        <MappingStep 
                            mappings={mappings} 
                            onMap={(k, v) => setMappings(prev => ({...prev, [k]: v}))}
                            ocrData={ocrData}
                        />
                    )}
                    {step === 4 && (
